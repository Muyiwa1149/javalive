...
**Canonical: `ProcessInvestmentPlanRoi`** (command signature `investments:process-roi`) — confirmed by reading `app/Console/Kernel.php` directly: it is the one actually scheduled (`->everyFiveMinutes()->withoutOverlapping()`), operating on the legacy `Investment`/`Plans`/`Tp_Transaction`/`User` tables that Plans is staying on. `ProcessInvestmentRoi` (signature `plans:process-roi`) is registered in `$commands` but **never scheduled** — dead code, not ported. `AutoTaskController::autotopup()`/`AutoRoiController::processAutomaticRoi()` are unreachable HTTP-triggered duplicates of the same math, also not ported. Algorithm ported 1:1 into a Spring `@Scheduled` job: trade_mode gate, weekend gate, interval-based next-payout calculation (regex-parsed "N Minutes/Hours/Days/Weeks/Months" plus named intervals), percentage/fixed profit calculation, investment completion + capital-return-if-configured, completion email.
