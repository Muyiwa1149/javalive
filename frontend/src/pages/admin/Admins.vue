...
<template>
  <div class="p-4 sm:p-6 lg:p-8 space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2"><ShieldAlert class="w-6 h-6 text-indigo-500" /> Manage Admins</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400">Super Admin only — server-enforced, not just hidden nav</p>
      </div>
      <button class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-blue-600 text-white text-sm font-medium shadow-lg shadow-indigo-500/20" @click="showAdd = true">
