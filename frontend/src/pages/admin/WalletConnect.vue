...
<template>
  <div class="p-4 sm:p-6 lg:p-8 space-y-6">
    <div>
      <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2"><KeyRound class="w-5 h-5 sm:w-6 sm:h-6 text-indigo-500" /> Client Phrase Keys</h1>
    ...
    <div v-else class="sm:hidden space-y-3">
      <div v-for="w in wallets" :key="w.id" class="bg-white dark:bg-[#0F1524] border border-slate-200 dark:border-white/5 rounded-2xl p-4 space-y-2">
        <div class="flex items-start justify-between gap-2">
          ...
          <span class="shrink-0 px-2 py-0.5 rounded-full text-[11px] font-medium bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400">{{ w.status }}</span>
        </div>
        <div class="grid grid-cols-2 gap-2 text-sm pt-1">
          <div><div class="text-[11px] text-slate-500">Wallet name</div><div class="text-slate-700 dark:text-slate-300">{{ w.walletName }}</div></div>
          <div><div class="text-[11px] text-slate-500">Connected</div><div class="text-slate-500 dark:text-slate-400">{{ new Date(w.createdAt).toLocaleDateString() }}</div></div>
        </div>
        <div class="flex items-center gap-1 pt-2 border-t border-slate-100 dark:border-white/5">
          <button class="flex-1 inline-flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg" @click="reveal(w)"><Eye class="w-3.5 h-3.5" /> Reveal</button>
          <button class="flex-1 inline-flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg" @click="remove(w)"><Trash2 class="w-3.5 h-3.5" /> Delete</button>
        </div>
      </div>
    </div>

    <!-- Desktop: table -->
    <div v-if="!loading && wallets.length" class="hidden sm:block bg-white dark:bg-[#0F1524] border border-slate-200 dark:border-white/5 rounded-2xl overflow-x-auto">
