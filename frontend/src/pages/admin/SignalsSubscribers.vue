...
import { Users2, AlertCircle } from 'lucide-vue-next'
...
onMounted(async () => {
  try {
    const { data } = await api.get('/admin/signal-provider/subscribers')
    subscribers.value = data
  } catch (e) {
    if (e.response?.status === 503) unavailable.value = true
    else errorMessage.value = e.response?.data?.message || 'Failed to load subscribers.'
  } finally {
    loading.value = false
  }
})
...
<template>
  <div class="p-4 sm:p-6 lg:p-8 space-y-6">
    <h1 class="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2"><Users2 class="w-6 h-6 text-indigo-500" /> Signal Subscribers</h1>
    <p class="text-sm text-slate-500 dark:text-slate-400">Proxies the external signal-subscribers API</p>
    ...
    <div v-else class="bg-white dark:bg-[#0F1524] border border-slate-200 dark:border-white/5 rounded-2xl p-6">
      <pre class="text-xs text-slate-600 dark:text-slate-300 whitespace-pre-wrap overflow-x-auto">{{ JSON.stringify(subscribers, null, 2) }}</pre>
    </div>
  </div>
</template>
