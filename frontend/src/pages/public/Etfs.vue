...
// The source blade's ETF markets table is populated client-side with no
// hardcoded fallback rows (only the DataTable shell and headers exist in
// the blade). No ETF listing data is available yet, so the table renders
// empty with the same "No matching records found" state the source used.
const etfs = []
...
const filteredEtfs = computed(() => {
  ...
  return etfs.filter((e) => {
    ...
    const matchesSearch = !q || e.symbol.toLowerCase().includes(q) || e.name.toLowerCase().includes(q)
    ...
    <div class="absolute inset-0 z-0 pointer-events-none opacity-20">
      <div class="absolute top-0 right-0 w-full h-full">
        <svg class="absolute top-0 right-0 w-full h-full" viewBox="0 0 800 800" xmlns="http://www.w3.org/2000/svg">
          ...
          <path fill="url(#etfGrad1)" d="M50,250 L150,200 L250,300 L350,150 L450,250 L550,100 L650,200 L750,150" stroke-width="3" stroke="rgba(59, 130, 246, 0.5)"/>
          <path fill="url(#etfGrad1)" d="M50,450 L750,450" stroke-width="1" stroke="rgba(59, 130, 246, 0.2)"/>
          <path fill="url(#etfGrad1)" d="M50,350 L750,350" stroke-width="1" stroke="rgba(59, 130, 246, 0.2)"/>
          <path fill="url(#etfGrad1)" d="M50,550 L750,550" stroke-width="1" stroke="rgba(59, 130, 246, 0.2)"/>
        </svg>
      </div>
      ...
      <div class="bg-gray-800 bg-opacity-50 backdrop-blur-sm rounded-xl border border-gray-700 overflow-hidden shadow-lg">
        ...
        <div class="p-4 border-b border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4">
          <div class="relative w-full md:w-64">
            <input v-model="searchQuery" type="text" class="w-full bg-gray-900 border border-gray-700 rounded-lg py-2 px-4 pl-10 text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Search ETFs...">
        ...
        <div class="relative overflow-x-auto" style="min-height: 200px;">
          <div class="w-full">
            <table class="w-full text-sm text-left text-gray-300">
              ...
              <tbody>
                <tr v-for="etf in filteredEtfs" :key="etf.symbol">
                  <td class="px-6 py-4">{{ etf.symbol }}</td>
                  <td class="px-6 py-4">{{ etf.name }}</td>
                  <td class="px-6 py-4">{{ etf.price }}</td>
                  <td class="px-6 py-4">{{ etf.change }}</td>
                  <td class="px-6 py-4">{{ etf.aum }}</td>
                  <td class="px-6 py-4">{{ etf.spread }}</td>
                  <td class="px-6 py-4"></td>
                </tr>
                <tr v-if="filteredEtfs.length === 0">
        ...
        <div class="space-y-8 order-1 lg:order-2">
          ...
          >
            <h3 class="text-2xl font-bold text-white mb-6">
              One of the primary goals of {{ settingsStore.settings?.siteName }} is to provide the best product in the market. Our relationships with leading tier one financial institutions mean deep liquidity and tighter spreads for Forex traders.
            </h3>
            ...
            <div class="mt-8">
              <RouterLink to="/about" :title="`Learn About ${settingsStore.settings?.siteName || ''}`" class="inline-flex items-center px-6 py-3 text-base font-medium text-white bg-blue-600 border border-transparent rounded-lg shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-blue-500 transition-all duration-200">
                Learn More
