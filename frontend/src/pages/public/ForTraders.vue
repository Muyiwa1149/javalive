...
// "More to Explore" cards — which card is currently hovered (mirrors the source's Alpine `activeCard`).
...
onMounted(async () => {
  ...
  // Port of the source's inline BreadcrumbList JSON-LD (used $settings->site_address / $settings->name).
  const origin = window.location.origin + '/'
  const siteName = settingsStore.settings?.siteName || ''
  ...
  ldScript.text = JSON.stringify({
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, item: { '@id': origin, name: siteName } },
      { '@type': 'ListItem', position: 2, item: { '@id': `${origin}for-traders`, name: 'For Traders' } },
    ],
  })
    ...
    <section class="py-16 bg-gray-800 relative">
      ...
      <div class="container mx-auto px-4 relative">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div class="relative h-full group" @mouseenter="activeCard = 1" @mouseleave="activeCard = null">
            <div class="absolute -inset-0.5 bg-gradient-to-r from-blue-600 to-emerald-500 rounded-xl blur opacity-30" :class="{ 'opacity-70': activeCard === 1 }"></div>
            <div class="relative bg-gray-900 bg-opacity-90 backdrop-filter backdrop-blur-sm p-6 rounded-xl border border-gray-700 h-full flex flex-col transition duration-300" :class="{ 'border-blue-500 transform scale-[1.02]': activeCard === 1 }">
              ...
              <div v-if="activeCard === 1" class="mt-6 pt-4 border-t border-gray-700 animate-fadeIn">
          ...
          <div class="relative h-full group" @mouseenter="activeCard = 2" @mouseleave="activeCard = null">
            <div class="absolute -inset-0.5 bg-gradient-to-r from-emerald-500 to-blue-600 rounded-xl blur opacity-30" :class="{ 'opacity-70': activeCard === 2 }"></div>
            <div class="relative bg-gray-900 bg-opacity-90 backdrop-filter backdrop-blur-sm p-6 rounded-xl border border-gray-700 h-full flex flex-col transition duration-300" :class="{ 'border-emerald-500 transform scale-[1.02]': activeCard === 2 }">
              ...
              <div v-if="activeCard === 2" class="mt-6 pt-4 border-t border-gray-700 animate-fadeIn">
                <a href="#" class="flex items-center justify-center text-emerald-400 hover:text-emerald-300 transition">
          ...
          <!-- Demo Account Card -->
          <div class="relative h-full group" @mouseenter="activeCard = 3" @mouseleave="activeCard = null">
            <div class="absolute -inset-0.5 bg-gradient-to-r from-purple-600 to-blue-500 rounded-xl blur opacity-30" :class="{ 'opacity-70': activeCard === 3 }"></div>
            <div class="relative bg-gray-900 bg-opacity-90 backdrop-filter backdrop-blur-sm p-6 rounded-xl border border-gray-700 h-full flex flex-col transition duration-300" :class="{ 'border-purple-500 transform scale-[1.02]': activeCard === 3 }">
              ...
              <div v-if="activeCard === 3" class="mt-6 pt-4 border-t border-gray-700 animate-fadeIn">
                <a href="#" class="flex items-center justify-center text-emerald-400 hover:text-emerald-300 transition">
          ...
          <div class="relative h-full group" @mouseenter="activeCard = 4" @mouseleave="activeCard = null">
            <div class="absolute -inset-0.5 bg-gradient-to-r from-blue-600 to-indigo-500 rounded-xl blur opacity-30" :class="{ 'opacity-70': activeCard === 4 }"></div>
            <div class="relative bg-gray-900 bg-opacity-90 backdrop-filter backdrop-blur-sm p-6 rounded-xl border border-gray-700 h-full flex flex-col transition duration-300" :class="{ 'border-indigo-500 transform scale-[1.02]': activeCard === 4 }">
              ...
              <div v-if="activeCard === 4" class="mt-6 pt-4 border-t border-gray-700 animate-fadeIn">
                <a href="#" class="flex items-center justify-center text-emerald-400 hover:text-emerald-300 transition">
        ...
        <div class="max-w-4xl mx-auto bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-sm rounded-xl p-8 border border-gray-700">
          <div class="text-center">
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-4">
              <RouterLink to="/register" class="px-6 py-3 bg-gradient-to-r from-blue-600 to-emerald-500 text-white font-medium rounded-lg transition duration-300 transform hover:scale-105 flex items-center justify-center">
                <span>Create Account</span>
                <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"></path>
                </svg>
              </RouterLink>
              <RouterLink to="/login" class="px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white font-medium rounded-lg transition duration-300 flex items-center justify-center">
                <span>Login</span>
                <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
