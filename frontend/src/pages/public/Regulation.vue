...
// Regulation and Licensing — ported from resources/views/home/regulation.blade.php
// The JSON-LD BreadcrumbList <script type="application/ld+json"> block is SEO
// metadata (not visual content) and depended on $settings->site_address,
// which has no equivalent in the SPA settings store yet, so it is skipped.
// The middle "Company" breadcrumb crumb pointed at {{$settings->site_address}}company,
// a page that doesn't exist in the router, so it's kept as inert text rather
// than a fabricated link. Alpine's `x-data="{ isHovered: false }"` on the
// trust-indicator cards was unused (hover visuals are pure Tailwind
// `group`/`group-hover:` CSS), so it is dropped rather than ported.
          ...
          <div class="bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-sm rounded-xl p-8 border border-gray-700">
            <div class="space-y-6">
              <div class="flex items-start space-x-4">
                ...
                <div>
                  <p class="text-gray-300 leading-relaxed">
                    {{ settingsStore.settings?.siteName }} with registration number 709718501 is a company registered under the Laws of Seychelles and is licensed by the Financial Services Authority (FSA) of Seychelles.
                  </p>
                </div>
              </div>
              ...
              <div class="flex items-start space-x-4">
                ...
                <div>
                  <p class="text-gray-300 leading-relaxed">
                    {{ settingsStore.settings?.siteName }} is a tradename of {{ settingsStore.settings?.siteName }} with registration number HE 19818009, which is registered as a Cyprus Investment Firm (CIF) and licensed by the Cyprus Securities and Exchange Commission (CySEC) in accordance with the Markets In Financial Instruments Directive (MiFID II).
                  </p>
                </div>
              </div>
            </div>
