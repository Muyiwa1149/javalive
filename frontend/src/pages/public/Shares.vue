...
// The source blade's Available Shares table only ships the header row —
// the tbody is populated client-side with no hardcoded fallback data, so
// there are no share rows to port. The table renders empty below, matching
// the source's "No matching records found" DataTable state.
const shares = []

const filteredShares = computed(() => {
  const q = searchQuery.value.trim().toLowerCase()
  if (!q) return shares
  return shares.filter((s) => s.symbol.toLowerCase().includes(q) || s.company.toLowerCase().includes(q))
})
</script>
          ...
          <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
              <li class="inline-flex items-center">
                <RouterLink to="/" class="text-gray-400 hover:text-white flex items-center">
        ...
        <div class="p-6 bg-gray-800 border-b border-gray-700">
          <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex items-center space-x-4">
              <!-- Search Box -->
              <div class="relative">
                <input v-model="searchQuery" type="text" class="w-64 px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-gray-200 focus:outline-none focus:border-blue-500 placeholder-gray-400"
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-700 text-gray-300">
              <tr v-for="share in filteredShares" :key="share.symbol">
                <td class="px-6 py-4">{{ share.symbol }}</td>
                <td class="px-6 py-4">{{ share.company }}</td>
                <td class="px-6 py-4">{{ share.price }}</td>
                <td class="px-6 py-4">{{ share.change }}</td>
                <td class="px-6 py-4">{{ share.volume }}</td>
              </tr>
          ...
          <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div class="text-center md:text-left">
              ...
              <div class="flex items-center justify-center md:justify-start space-x-4">
                <a v-if="settingsStore.settings?.contactEmail" :href="`mailto:${settingsStore.settings.contactEmail}`" class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-700 hover:bg-gray-600 transition-colors duration-300">
            ...
            <div class="text-center md:text-right">
              ...
              <RouterLink to="/contact" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg text-white bg-gradient-to-r from-blue-600 to-emerald-500 hover:from-blue-700 hover:to-emerald-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-300 shadow-lg hover:shadow-xl">
